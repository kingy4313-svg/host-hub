...
function Media({
  url, type, alt, className, item,
}: { url?: string; type?: string; alt?: string; className?: string | undefined; item?: any }) {
  const u = (item?.mediaUrl || url || "").trim();
  const t = item?.mediaType || type;

  const isYouTube = /youtu\.be|youtube\.com/.test(u);

  const isVideoExt = /\.(mp4|webm|ogv|ogg|mov|m4v)(\?|#|$)/i.test(u) || u.startsWith("blob:") || u.startsWith("data:video");

  if (t !== "video" && !isVideoExt && !isYouTube) {
    return <img src={u} alt={alt || item?.label || ""} loading="lazy" className={className} />;
  }

  if (isYouTube) {
    // try common patterns to extract the 11-char video id
    let videoId: string | null = null;
    const m = u.match(/(?:v=|vi=|youtu\.be\/|embed\/)([^&?#\/]{11})/);
    if (m && m[1]) videoId = m[1];
    if (!videoId) {
      const seg = u.split(/[\/?#&]/).find((s: string | any[]) => s.length === 11);
      if (seg) videoId = seg;
    }
    ...
    return (
      <iframe
        ...
        title={item?.label || alt || "YouTube video"}
  ...
  // fallback to native video player for file URLs
  const isWebm = /\.(webm)(\?|#|$)/i.test(u);
  const mime = isWebm ? "video/webm" : "video/mp4";
  return (
    <video controls width="100%" poster={item?.posterUrl} className={className}>
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 backdrop-blur-md bg-background/40">
      <nav className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
        <Link to="/" className="logo-glow flex items-center gap-3 font-display text-xl font-bold tracking-wide">
...
export function Intro() {
  ...
  return (
    <section id="about" className="px-6 py-20 text-center">
      ...
      <AnimatedIconGrid
        className="mx-auto mt-14 grid max-w-4xl grid-cols-1 gap-y-8 sm:grid-cols-2 lg:grid-cols-3"
        itemClassName="flex min-h-[92px] flex-col items-center justify-start gap-3 text-center"
        items={intro.stats.map((s) => ({
          ...
          label: (
            <span className="flex flex-wrap items-center justify-center gap-1 font-display text-sm font-bold leading-tight tracking-wide text-gold">
              {s.value ? <CountUp value={s.value} className="whitespace-nowrap" /> : null}
              {s.value && s.label ? <span className="whitespace-nowrap">{s.label}</span> : null}
              {!s.value && s.label ? <span className="whitespace-nowrap">{s.label}</span> : null}
            </span>
          ),
        }))}
...
export function FeaturedMoments() {
  ...
  const scroll = (dir: number) => {
    const amount = Math.min(ref.current?.clientWidth ?? 320, 340) * 0.85;
    ref.current?.scrollBy({ left: dir * amount, behavior: "smooth" });
  };
  return (
    <section className="overflow-hidden py-12 sm:py-16 md:py-20">
      <h2 className="px-4 text-center font-display text-[clamp(1.75rem,4.5vw,3rem)] font-bold sm:px-6">
        {featured.headingWhite} <span className="text-gold-gradient">{featured.headingGold}</span>
      </h2>
      <span className="gold-divider mt-5" />
      {featured.showArrows ? (
        <div className="mx-auto mt-8 flex max-w-6xl justify-between px-4 sm:mt-10 sm:px-6">
          <button
            onClick={() => scroll(-1)}
            aria-label="Previous"
            className="touch-target flex items-center justify-center rounded-full border border-gold/40 text-gold"
          >
      ...
      <div
        ref={ref}
        className="no-scrollbar mt-4 flex snap-x snap-mandatory gap-4 overflow-x-auto px-4 pb-4 sm:mt-6 sm:gap-6 sm:px-6 md:px-[max(1.5rem,calc(50vw-32rem))]"
      >
        {featured.items.map((m) => (
          <figure
            key={m.id}
            className="relative w-[min(85vw,18.75rem)] shrink-0 snap-start overflow-hidden rounded-2xl border border-border sm:w-[min(75vw,18.75rem)]"
          >
            <div className="px-3 pt-3 text-center font-display text-xs font-bold sm:px-4 sm:text-sm">
              {m.label}
            </div>
            ...
            <div className="relative">
              <Media
                ...
                type={m.mediaType}
                alt={m.caption}
                className="h-[clamp(16rem,55vw,27.5rem)] w-full object-cover"
              />
            </div>
...
function WorkCard({
  ...
  const handleKey = (e: React.KeyboardEvent) => {
    if (e.key === "Enter" || e.key === " ") {
      e.preventDefault();
      if (effectiveUrl) onPlay(effectiveUrl, item.title);
    }
  };
  return (
    <article
      ...
      onKeyDown={handleKey}
      onClick={() => { if (effectiveUrl) onPlay(effectiveUrl, item.title); }}
      className={effectiveUrl ? "cursor-pointer" : undefined}
      ...
      <>
        <div className="relative overflow-hidden rounded-lg border border-border">
          {ytMatch ? (
            ...
            <div className="relative min-h-[12rem] rounded-xl border border-border bg-black">
              <div className="absolute inset-0 flex flex-col items-center justify-center p-4 text-center">
                <p className="font-display text-sm text-muted-foreground">Video failed to load.</p>
                <a href={effectiveUrl} target="_blank" rel="noopener noreferrer" className="mt-2 text-gold underline">Open video in new tab</a>
              </div>
            </div>
          ) : (
            <img src={thumbSrc} alt={item.title} loading="lazy" className="h-48 w-full object-cover" />
          )}
          {item.duration ? <span className="absolute bottom-2 right-2 rounded bg-background/85 px-2 py-0.5 text-xs">{item.duration}</span> : null}
          <Play className="absolute inset-0 m-auto size-10 text-gold opacity-80" />
        </div>
        <h4 className="mt-3 font-display text-sm font-bold">{item.title}</h4>
        <p className="text-xs text-gold">{item.category || activeName}</p>
        {effectiveUrl ? (
          <div className="mt-2">
            <button
              onClick={(e) => { e.stopPropagation(); onPlay(effectiveUrl, item.title); }}
              className="btn-gold inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs"
            >
              Play on site
            </button>
            {errored ? (
              <div className="mt-2 text-xs">
                <a href={effectiveUrl} target="_blank" rel="noopener noreferrer" className="text-gold underline">Open video in new tab</a>
              </div>
            ) : null}
          </div>
        ) : null}
    ...
    <section id="works" className="px-6 py-20">
      ...
      <div className="mx-auto mt-12 max-w-6xl">
        ...
        <ScrollRevealGroup key={active?.id} className="mt-6 grid gap-8 sm:grid-cols-2 lg:grid-cols-3" stagger={0.08} amount={0.15}>
          {(active?.items ?? []).map((w) => {
            const findDefaultVideo = (id: string) => {
              for (const tab of DEFAULT_CONTENT.works.tabs) {
                const it = tab.items.find((x) => x.id === id);
                if (it && it.videoUrl) return it.videoUrl;
              }
              return "";
            };
            const effectiveUrl = (w.videoUrl && w.videoUrl.trim()) ? w.videoUrl.trim() : findDefaultVideo(w.id);
            const isFileVideo = !!effectiveUrl && /\.(mp4|webm|ogv|ogg|mov|m4v)(\?|#|$)/i.test(effectiveUrl);
            const ytMatch = effectiveUrl ? effectiveUrl.match(/(?:youtube\.com.*v=|youtu\.be\/)([a-zA-Z0-9_-]{11})/) : null;
            let thumbSrc = w.thumbUrl;
            if (!thumbSrc && ytMatch) thumbSrc = `https://img.youtube.com/vi/${ytMatch[1]}/hqdefault.jpg`;
            return (
              <RevealItem key={w.id}>
                <WorkCard
                  item={w}
                  ...
                  onPlay={(url: any, title: any) => setPlaying({ url, title })}
                />
              </RevealItem>
            );
          })}
